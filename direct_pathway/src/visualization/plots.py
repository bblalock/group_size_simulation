...
def plotly_theme_decorator(func):
    """
    Decorator that applies a consistent theme to plotly figures including legend position and font settings.
    ...
    --------
    callable
        Decorated function that returns a styled figure
    """
    def wrapper(*args, **kwargs):
        # Extract height and width from kwargs if provided, otherwise use defaults
...
def calculate_deviation_metrics(data):
    ...
    df['disadv_delta_from_avg_percent'] = (df['rate_disadv'] - df['pop_avg']) / df['pop_avg'] * 100
    df['adv_delta_from_avg_percent'] = (df['rate_adv'] - df['pop_avg']) / df['pop_avg'] * 100
    ...
    df['disparity_ratio_label'] = df['disparity_ratio_rounded'].apply(create_disparity_ratio_label)
...
def create_explanatory_visual(standard_sim_data, exemplar_pop_avg=200, exemplar_prop_disadv=[0.1, 0.5, 0.9], height=1000, width=1000, relative=False):
    ...
    filtered_explanatory_data = standard_sim_data[standard_sim_data['pop_avg'] == exemplar_pop_avg]
    filtered_explanatory_data = filtered_explanatory_data[filtered_explanatory_data['prop_disadv'].isin(exemplar_prop_disadv)]

    if relative:
        # Calculate deviation metrics if plotting relative values
        filtered_explanatory_data = calculate_deviation_metrics(filtered_explanatory_data)
        
        # Create melted dataframe with percent deviations
        melted_df = pd.melt(
            filtered_explanatory_data,
            id_vars=['prop_disadv', 'disparity_ratio'],
            value_vars=['adv_delta_from_avg_percent', 'disadv_delta_from_avg_percent'],
            var_name='group',
            value_name='percent_deviation'
        )
        melted_df['group'] = melted_df['group'].str.replace('_delta_from_avg_percent', '')
...
@plotly_theme_decorator
def plot_3d_simulation_results(standard_sim_data, height=800, width=1000):
    """
    ...
    y_min, y_max = min(data['disadv_delta_from_avg_percent']), max(data['disadv_delta_from_avg_percent'])
    ...
    plane = go.Surface(
        ...
        y=y_grid,
        z=z_grid,
        colorscale=[[0, 'rgba(0,0,0,0.2)'], [1, 'rgba(0,0,0,0.2)']],  # Semi-transparent black
        showscale=False,
        name='x=0.5'
    )
...
def plot_prop_disadv_to_bias_by_ratio(data, height=800, width=1000, n_std=1):
    ...
    grouped_data = data.groupby(['prop_disadv', 'disparity_ratio_rounded'])['bias_parameter'].agg(['mean', 'std']).reset_index()
    ...
    for i, disparity_ratio in enumerate(rounded_ratios):
        subset = grouped_data[grouped_data['disparity_ratio_rounded'] == disparity_ratio]
        ...
        fig.add_trace(
            go.Scatter(
                x=subset['prop_disadv'].tolist() + subset['prop_disadv'].tolist()[::-1],  # x, then x reversed
                y=subset['upper'].tolist() + subset['lower'].tolist()[::-1],  # upper, then lower reversed
                fill='toself',
                fillcolor=color,
                line=dict(color=color),
                hoverinfo="skip",
...
def plot_disadv_deviation_from_avg(data, height=800, width=800, n_std=1):
    ...
    # Group by prop_disadv and disparity_ratio (rounded) to calculate mean and std for confidence bands
    grouped_data = data.groupby(['prop_disadv', 'disparity_ratio_rounded'])['disadv_delta_from_avg_percent'].agg(['mean', 'std']).reset_index()
...
@plotly_theme_decorator
def plot_disadv_deviation_boxplot(standard_sim_data, height=700, width=1200):
    """
    ...
    standard_sim_data['disadv_delta_from_avg'] = standard_sim_data['rate_disadv'] - standard_sim_data['pop_avg']
    
    # Calculate the percent deviation from average
    standard_sim_data['disadv_delta_from_avg_percent'] = (standard_sim_data['rate_disadv'] - standard_sim_data['pop_avg']) / standard_sim_data['pop_avg'] * 100
    
    # Round the disparity ratio for grouping
    standard_sim_data['disparity_ratio_rounded'] = standard_sim_data['disparity_ratio'].apply(np.round)
    ...
    standard_sim_data['disparity_ratio_label'] = standard_sim_data['disparity_ratio_rounded'].apply(create_disparity_label)
    
    # Create bins for prop_disadv (4 groups split at 50%)
    bins = [0, 0.25, 0.5, 0.75, 1.0]
    labels = ['Small Minority (0-25%)', 'Minority (26-50%)', 'Majority (51-75%)', 'Large Majority (76-100%)']
    ...
    fig = px.box(
        data_frame=standard_sim_data,
        x='prop_disadv_binned',
        y='disadv_delta_from_avg_percent',
        color='disparity_ratio_label',
